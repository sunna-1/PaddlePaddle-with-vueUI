...
- 📊 **多维度车流量统计**：支持车辆总数、唯一ID车辆数、道路平均车辆密度、车辆面积占比等指标统计
- 🚦 **混合路况分析器**：整合HSV橙色框车辆计数 + 帧差法运动分析，智能判定道路为畅通/缓行/拥堵三类状态
- 📈 **可视化结果展示**：提供带检测标注的视频回放、车流密度/车辆位移趋势图表展示
- 🎯 **实时路况占比**：环形图展示畅通、缓行、拥堵的帧占比分布
- 🔍 **判断因素分析**：展示路况判断的各指标信号和置信度，提供透明可解释的判断依据
...
- **混合路况分析器**：
  - HSV橙色框车辆计数：从检测输出中统计车辆密度
  - 帧差法运动分析：分析原始视频中车辆运动强度
  - 多规则融合判断：基于密度、运动强度、拥堵帧占比等综合判定
...
│   ├── deploy/         # 部署脚本
│   │   └── pipeline/   # PP-Vehicle工具（含检测数据导出改造）
│   │       └── config/ # 配置文件
│   ├── configs/        # 模型配置
│   └── output/        # 输出目录（含 *_detection.json）
├── docs/               # 技术文档
├── video-demo/         # 测试视频
├── app.py             # 后端主程序（含路况分析引擎）
├── config.json        # 配置文件
...
1. 上传视频后，系统自动开始分析
2. 分析完成后，数据面板显示统计结果：
   - **概览卡片**：检测车辆总数、唯一车辆数、平均每帧车辆、路况参考判断及置信度
   - **详细指标**：车辆密度（同帧最大/最小/平均）、车辆位移（平均帧间位移、静止车辆占比）、跟踪时长（平均/最长跟踪帧数）、车辆面积（平均/最大面积占比）
   - **判断因素分析**：展示路况判断的五个指标信号（车辆面积占比、近处车辆占比、稳定跟踪速度、静止车辆比例、最大同帧数），每项标注为畅通/缓行/拥堵信号
   - **趋势图表**：车流密度趋势折线图、车辆位移趋势折线图、路况帧占比环形图
3. 可以下载带检测标注的输出视频

> 详细技术说明请参阅 [路况分析模块技术文档](docs/路况分析模块技术文档.md)
...
本项目采用创新的**混合路况分析器**，结合多种信号源进行准确判断：

**算法特点**：
- 🎯 **双数据源分析**：同时利用PP-Vehicle检测结果和原始视频
- 📊 **HSV车辆计数**：基于检测输出中的橙色框统计车辆密度
- 🎯 **运动强度分析**：使用帧差法计算车辆运动速度和变化
- 🔀 **多规则融合**：多条件加权判断，提高鲁棒性

**判断逻辑优先级**：
1. **HSV拥堵帧占比**：如果检测输出中拥堵帧占比>80%且密度≥12 → 拥堵
2. **运动强度骤降**：密度≥8且后半段运动强度骤降→ 拥堵
3. **密度低且运动强**：→ 畅通
4. **中等密度且运动强**：→ 畅通
5. **其他情况**：→ 缓行

**核心参数**：
- 车辆密度阈值：低<5，中等5-12，高≥12
- 运动强度变化：后半段/前半段<0.5视为骤降
- 静止判断：帧间位移<2像素视为静止
...
### v2.0.0 (2026-05-03)
- **新增混合路况分析器**：整合HSV橙色框计数 + 帧差法运动分析
- **优化路况判断算法**：基于密度、运动强度、拥堵帧占比的多规则融合
- **新增`app_ultimate.py`**：包含最终版路况分析引擎
- **添加pp-vehicle打包目录**：项目核心代码和文档打包
- **更新README文档**：详细说明混合路况分析器原理

### v1.1.0 (2026-04-22)
- 重构路况分析模块：基于Pipeline检测数据JSON替代HSV后备分析
- 新增多指标加权投票路况判断引擎（面积占比、近处车辆比例、稳定跟踪速度等）
- 新增判断因素分析展示，提供透明可解释的判断依据和置信度
- 新增车辆面积占比、稳定跟踪速度等统计指标
- 修复中文文件名上传覆盖问题（UUID安全文件名）
- 修复psutil CPU使用率首次调用返回0的问题
- 优化Pipeline数据导出格式（精简JSON，紧凑序列化）
- 重构前端数据面板，未分析时也展示数据模块框架
- 新增路况分析模块技术文档
